...
// Precedencia: variable explícita > APP_URL de la aplicación.
// No hay valor por defecto a propósito: un entorno por defecto hace que una
// configuración incompleta acabe ejecutando contra el sitio equivocado en
// silencio. Sin URL, global-setup se detiene con un error explicativo.
const detected = process.env.BASE_URL ? null : appUrlFromLaravel();
export const BASE_URL = process.env.BASE_URL || detected || '';
...
export const IS_LOCAL_ENV = BASE_URL
  ? /(\.test$)|^localhost$|^127\.0\.0\.1$/.test(new URL(BASE_URL).hostname)
  : false;
